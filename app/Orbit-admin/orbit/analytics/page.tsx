      ...
      <div className="flex flex-wrap gap-3">
        {segments.map(s => (
          <div key={s.label} className="flex items-center gap-1.5">
            ...
            <span className="text-[10.5px] text-[#5c5a55]">
...
export default async function OrbitAnalyticsPage() {
  ...
  const emailSegments = [
    { label: "sent",    value: emailMap.get("sent")    ?? 0, color: "#059669" },
    { label: "queued",  value: emailMap.get("queued")  ?? 0, color: "#2383e2" },
    { label: "sending", value: emailMap.get("sending") ?? 0, color: "#7c3aed" },
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ── Header ── */}
      <div className="relative overflow-hidden rounded-[24px] bg-gradient-to-br from-[#92400e] via-[#b45309] to-[#d97706] p-8 shadow-[0_8px_32px_rgba(180,83,9,0.25)]">
        ...
        <div className="pointer-events-none absolute -right-20 -top-20 size-80 rounded-full bg-yellow-300/20 blur-[80px]" />
        <div className="relative flex items-center justify-between gap-6">
          <div>
            <p className="text-[10.5px] font-bold uppercase tracking-[0.14em] text-white/50">Orbit Admin</p>
            <h1 className="mt-1 text-[32px] font-black tracking-tight text-white">Analytics</h1>
            <p className="mt-1 text-[13px] text-white/55">Platform-wide metrics with real-time data and 30-day trends.</p>
          </div>
          <div className="hidden shrink-0 items-center gap-5 rounded-[16px] border border-white/[0.15] bg-white/[0.08] px-6 py-4 sm:flex">
            <div className="text-center">
              <p className="text-[28px] font-black text-white">{activationRate}%</p>
              <p className="text-[9.5px] font-bold uppercase tracking-widest text-white/50">Activation rate</p>
            </div>
            <div className="h-8 w-px bg-white/15" />
            <div className="text-center">
              <p className="text-[28px] font-black text-white">{activeSessions!.count}</p>
              <p className="text-[9.5px] font-bold uppercase tracking-widest text-white/50">Active sessions</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        {[
          { label: "Total users",     value: totalUsers!.count,      sub: "All registered",       color: "#2383e2", icon: "👤" },
          { label: "New users (7d)",  value: newUsers7d!.count,      sub: userTrendPct !== null ? `${userTrend >= 0 ? "+" : ""}${userTrendPct}% vs prev 7d` : "This week", color: "#7c3aed", icon: "📈" },
          { label: "Workspaces",      value: totalWorkspaces!.count, sub: "Active tenants",        color: "#059669", icon: "🏢" },
          { label: "Total sessions",  value: totalSessions!.count,   sub: `${activeSessions!.count} active now`, color: "#f59e0b", icon: "⚡" },
        ].map(s => (
          <div key={s.label}
            className="flex flex-col gap-3 rounded-[18px] border border-black/[0.06] bg-white p-5 shadow-[0_1px_4px_rgba(0,0,0,0.05)]">
            <div className="flex items-center justify-between">
              <span className="text-[18px]">{s.icon}</span>
              {s.label === "New users (7d)" && userTrendPct !== null && (
                <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${
                  userTrend >= 0 ? "bg-emerald-50 text-emerald-700" : "bg-red-50 text-red-700"
                }`}>{userTrend >= 0 ? "↑" : "↓"} {Math.abs(userTrendPct)}%</span>
              )}
            </div>
            <div>
              <p className="text-[30px] font-black leading-none" style={{ color: s.color }}>{s.value}</p>
              <p className="mt-1 text-[12px] font-bold text-[#37352f]">{s.label}</p>
              <p className="text-[10.5px] text-[#a8a29e]">{s.sub}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <ChartCard
          ...
          <BarChart data={userSignups30d} color="#2383e2" />
          <DayLabels days={30} />
        </ChartCard>
        ...
        <ChartCard
          ...
          <BarChart data={wsGrowth30d} color="#059669" />
          <DayLabels days={30} />
        </ChartCard>
        ...
        <ChartCard
          ...
          <div className="mt-2 grid grid-cols-4 gap-2">
            {emailSegments.map(s => (
              <div key={s.label} className="rounded-[10px] p-3 text-center" style={{ background: `${s.color}10` }}>
                <p className="text-[18px] font-black" style={{ color: s.color }}>{s.value}</p>
                <p className="text-[10px] font-semibold text-[#a8a29e]">{s.label}</p>
              </div>
            ))}
          </div>
        </ChartCard>
          ...
          <div className="mt-2 space-y-4">
            ...
            <div>
              <div className="mb-1.5 flex items-center justify-between">
                <span className="text-[11px] font-semibold text-[#5c5a55]">Users → Workspaces</span>
                <span className="text-[11px] font-black text-[#7c3aed]">{activationRate}%</span>
              </div>
              <div className="h-2.5 overflow-hidden rounded-full bg-[#f5f4f2]">
                <div className="h-full rounded-full bg-gradient-to-r from-[#7c3aed] to-[#c084fc] transition-all"
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                { label: "7-day signups",  value: newUsers7d!.count,  color: "#7c3aed", pct: totalUsers!.count > 0 ? Math.round((newUsers7d!.count / totalUsers!.count) * 100) : 0 },
                { label: "30-day signups", value: newUsers30d!.count, color: "#2383e2", pct: totalUsers!.count > 0 ? Math.round((newUsers30d!.count / totalUsers!.count) * 100) : 0 },
              ].map(r => (
                <div key={r.label} className="rounded-[12px] border border-black/[0.06] bg-[#faf9f8] p-3">
                  <p className="text-[20px] font-black" style={{ color: r.color }}>{r.value}</p>
                  <p className="text-[10.5px] font-medium text-[#787774]">{r.label}</p>
                  <div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-[#ece9e5]">
                    <div className="h-full rounded-full" style={{ width: `${r.pct}%`, background: r.color }} />
                  </div>
                  <p className="mt-1 text-[9.5px] text-[#c4c1bb]">{r.pct}% of all users</p>
                </div>
              ))}
            </div>
            {/* Trend indicator */}
            {userTrendPct !== null && (
              <div className={`flex items-center gap-2 rounded-[10px] p-3 ${userTrend >= 0 ? "bg-emerald-50" : "bg-red-50"}`}>
                <span className={`text-[16px] font-black ${userTrend >= 0 ? "text-emerald-600" : "text-red-600"}`}>
                  {userTrend >= 0 ? "↑" : "↓"} {Math.abs(userTrendPct)}%
                </span>
                <span className={`text-[11px] ${userTrend >= 0 ? "text-emerald-700" : "text-red-700"}`}>
...
function ChartCard({
  ...
  return (
    <div className="overflow-hidden rounded-[18px] border border-black/[0.06] bg-white shadow-[0_2px_8px_rgba(0,0,0,0.06)]">
      {/* Top accent */}
      <div className="h-[3px]" style={{ background: `linear-gradient(90deg, ${color}, ${color}55)` }} />
      <div className="p-5">
        <div className="flex items-start justify-between gap-3">
          <div>
            <h3 className="text-[13.5px] font-bold text-[#1c1917]">{title}</h3>
            <p className="text-[11px] text-[#a8a29e]">{subtitle}</p>
          </div>
          <div className="text-right shrink-0">
            <p className="text-[22px] font-black leading-none" style={{ color }}>{value}</p>
            <p className="text-[10px] text-[#a8a29e]">{valueLabel}</p>
            {badge && <p className="mt-0.5 text-[9.5px] text-[#c4c1bb]">{badge}</p>}
          </div>
        </div>
...
function DayLabels({ days }: { days: number }) {
  ...
  return (
    <div className="mt-1.5 flex justify-between">
      <span className="text-[9.5px] text-[#c4c1bb]">{fmt(start)}</span>
      <span className="text-[9.5px] text-[#c4c1bb]">{fmt(now)}</span>
    </div>
  );
}
