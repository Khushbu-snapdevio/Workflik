...
async function resolveBlock(blockId: string, userId: string) {
  ...
  const [page] = await db.select({ workspaceId: pages.workspaceId, isDeleted: pages.isDeleted })
    .from(pages).where(eq(pages.id, block.pageId)).limit(1);
...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!parsed.success) return apiError(400, "Invalid block update");

    const updates: Record<string, unknown> = {};
    if (parsed.data.type       !== undefined) updates.type          = parsed.data.type;
    if (parsed.data.content    !== undefined) updates.content       = parsed.data.content;
    if (parsed.data.orderIndex !== undefined) updates.orderIndex    = parsed.data.orderIndex;
    if (parsed.data.parentBlockId !== undefined) updates.parentBlockId = parsed.data.parentBlockId;
