...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const views = await db.select().from(databaseViews).where(eq(databaseViews.databaseId, id)).orderBy(asc(databaseViews.orderIndex));
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [view] = await db.insert(databaseViews).values({
    ...
    filters:             (body.filters as typeof databaseViews.$inferInsert["filters"]) ?? [],
    ...
    cardDisplayProps:    (body.cardDisplayProps as typeof databaseViews.$inferInsert["cardDisplayProps"]) ?? [],
