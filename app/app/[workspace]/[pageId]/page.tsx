...
export default async function PageEditorPage({ params }: Props) {
  ...
  const statusBanner = (
    <>
      ...
      {page.isLocked && (
        <div className="mb-5 flex items-center gap-3 rounded-[var(--radius-md)] border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-700">
      ...
      <div className="flex h-11 shrink-0 items-center justify-between border-b border-border/60 bg-card/95 px-3 backdrop-blur-sm">
        ...
        <nav className="flex min-w-0 items-center gap-0.5 text-xs">
          ...
          {breadcrumbs.map((crumb) => (
            <span key={crumb.id} className="flex min-w-0 items-center gap-0.5">
              ...
              <a
                ...
                className="max-w-[120px] truncate rounded-[var(--radius-sm)] px-2 py-1 text-muted-foreground transition-colors hover:bg-accent hover:text-foreground"
              >
          ...
          <span className="flex min-w-0 items-center gap-0.5">
            ...
            <span className="max-w-[240px] truncate px-2 py-1 font-semibold text-foreground/80">
              {page.title || "Untitled"}
            </span>
          </span>
        </nav>
