...
const ACTION_META: Record<string, { label: string; pill: string; iconCls: string; icon: React.ReactNode }> = {
  "user.signup": {
    label: "User signed up", pill: "bg-success/10 text-success", iconCls: "bg-success/10 text-success",
    icon: <svg viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="size-3.5"><circle cx="5" cy="4.5" r="2"/><path d="M1 12c0-2 1.7-3.5 3.5-3.5S9 10 9 12"/><path d="M11 5v4M9 7h4"/></svg>,
  },
  "user.login": {
    label: "User login", pill: "bg-muted text-muted-foreground", iconCls: "bg-muted text-muted-foreground",
    icon: <svg viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="size-3.5"><circle cx="5" cy="4.5" r="2"/><path d="M1 12c0-2 1.7-3.5 3.5-3.5S9 10 9 12"/></svg>,
  },
  "user.banned": {
    label: "User banned", pill: "bg-destructive/5 text-destructive", iconCls: "bg-destructive/5 text-destructive",
  ...
  "workspace.created": {
    label: "Workspace created", pill: "bg-primary/10 text-primary", iconCls: "bg-primary/10 text-primary",
    icon: <svg viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="size-3.5"><rect x="1.5" y="1.5" width="11" height="11" rx="1.5"/><path d="M5 7h4M7 5v4"/></svg>,
  },
  "workspace.updated": {
    label: "Workspace updated", pill: "bg-warning/10 text-warning", iconCls: "bg-warning/10 text-warning",
    icon: <svg viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="size-3.5"><rect x="1.5" y="1.5" width="11" height="11" rx="1.5"/><path d="M4.5 7h5M4.5 9.5h3"/></svg>,
  },
  "workspace.deleted": {
    label: "Workspace deleted", pill: "bg-destructive/5 text-destructive", iconCls: "bg-destructive/5 text-destructive",
    icon: <svg viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="size-3.5"><rect x="1.5" y="1.5" width="11" height="11" rx="1.5"/><path d="M4.5 4.5l5 5M9.5 4.5l-5 5"/></svg>,
  },
  "workspace.force_deleted": {
    label: "Workspace force-deleted", pill: "bg-destructive/5 text-destructive", iconCls: "bg-destructive/5 text-destructive",
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Platform health, recent registrations, and operator actions.</p>
        </div>
            ...
            {recentUsers.map((u) => {
              ...
              return (
                <Link key={u.id} href={`/orbit-admin/orbit/users/${u.id}`}
                  className="group flex items-center gap-3.5 px-5 py-3 transition-colors hover:bg-accent">
                  <div className="relative shrink-0">
                    <span className={`flex size-7 items-center justify-center rounded-full text-xs font-semibold text-white ${avatarBg(u.id)}`}>
                      {avatarChar}
                    </span>
                    {u.banned && (
                      <span className="absolute -right-0.5 -top-0.5 flex size-2.5 items-center justify-center rounded-full border-2 border-card bg-destructive" />
                    )}
                  </div>
                  <div className="min-w-0 flex-1">
