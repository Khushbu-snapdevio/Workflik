...
export default async function DashboardPage() {
 ...
 return (
  ...
  >
   ...
   <div className="mb-6">
    <div className="mb-2">
     <span className="inline-flex items-center gap-1.5 rounded-full bg-primary/10 px-2.5 py-0.5 text-xs font-semibold tracking-wide text-primary">
      <LayoutGrid size={9} />
      Workspace
     </span>
    </div>
   ...
   <div className="mb-5 grid gap-4 md:grid-cols-3">
    ...
    <div className="rounded-[var(--radius-lg)] border border-border bg-card">
     ...
     <div className="space-y-3 px-5 py-4">
      <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-semibold ${isVerified ? "bg-success/10 text-success" : "bg-warning/10 text-warning"}`}>
    ...
    <div className="rounded-[var(--radius-lg)] border border-border bg-card">
     ...
     <div className="space-y-3 px-5 py-4">
      <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-semibold capitalize ${isAdmin ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground"}`}>
       {freshUser?.role ?? "user"}
      </span>
      <p className="text-xs leading-relaxed text-muted-foreground">
    ...
    <div className="rounded-[var(--radius-lg)] border border-border bg-card">
     ...
     <div className="space-y-3 px-5 py-4">
      <Link
       ...
       className="inline-flex h-8 items-center gap-1.5 rounded-[var(--radius-sm)] bg-primary px-4 text-xs font-semibold text-primary-foreground transition-colors duration-150 hover:bg-primary/90"
      >
       Edit profile
      </Link>
      <p className="text-xs leading-relaxed text-muted-foreground">
   ...
   <div className="rounded-[var(--radius-lg)] border border-border bg-card">
    <div className="flex items-center gap-2.5 border-b border-border/60 px-5 py-3">
     ...
     <span className="text-sm font-semibold text-foreground">Recent Email Outbox</span>
     <span className="ml-auto text-xs text-muted-foreground/50">Latest transactional emails</span>
    </div>

    {/* Column headers */}
    <div className="grid grid-cols-[1fr_96px_168px] gap-4 border-b border-border/40 px-5 py-2">
     <span className="text-xs font-semibold tracking-wide text-muted-foreground/60">Subject</span>
     <span className="text-xs font-semibold tracking-wide text-muted-foreground/60">Status</span>
    ...
    {emails.length === 0 ? (
     <div className="flex items-center gap-4 px-5 py-8">
      ...
      <div>
       ...
       <p className="text-xs text-muted-foreground/60">Transactional emails will appear here.</p>
      </div>
     </div>
    ) : (
     <div className="divide-y divide-border/40">
      {emails.map((email) => (
       ...
       >
        ...
        <span>
         <span className={`inline-flex items-center gap-1.5 rounded-full px-2 py-0.5 text-xs font-semibold ${email.status === "sent" ? "bg-success/10 text-success" : "bg-warning/10 text-warning"}`}>
