...
export function EntrySidePanel({
  ...
  return (
    <>
      ...
      <div
        ref={panelRef}
        className="fixed right-0 top-0 z-50 flex h-full w-[460px] flex-col overflow-hidden bg-card border-l border-primary/20 shadow-[−8px_0_60px_rgba(0,0,0,0.18)]"
        style={{ animation: "sidePanelIn 0.25s cubic-bezier(0.22,1,0.36,1)" }}
      >

        {/* ── Top nav bar ─────────────────────────────────────────────────── */}
        <div className="flex shrink-0 items-center justify-between bg-card/80 px-3 py-2.5 shadow-[0_1px_0_rgba(0,0,0,0.06)] backdrop-blur-md dark:bg-card/5">
          <button
            onClick={onClose}
            className="flex items-center gap-1.5 rounded-[var(--radius-sm)] px-2 py-1.5 text-xs font-medium text-muted-foreground/60 transition-colors hover:bg-black/5 hover:text-foreground dark:hover:bg-card/8"
          >
          ...
          <div className="flex items-center gap-1">
            {isEditor && (
              <button
                ...
                className="flex size-8 items-center justify-center rounded-[var(--radius-sm)] text-muted-foreground/40 transition-colors hover:bg-red-50 hover:text-red-500 dark:hover:bg-red-950/40"
                title="Delete entry"
              >
                <Trash size={14} />
              </button>
            ...
            <button
              onClick={onClose}
              className="flex size-8 items-center justify-center rounded-[var(--radius-sm)] text-muted-foreground/40 transition-colors hover:bg-black/6 hover:text-foreground dark:hover:bg-card/8"
              title="Close  (Esc)"
            >
              <X size={15} />
            </button>
        ...
        {/* ── Hero ────────────────────────────────────────────────────────── */}
        <div className="relative shrink-0 overflow-hidden bg-gradient-to-b from-primary/[0.06] to-transparent">

          <div className="relative px-5 pt-5 pb-4">
            {/* Icon */}
            <div className="mb-4">
              {entry.icon ? (
                <span className="inline-block rounded-[var(--radius-lg)] bg-card/60 p-2 text-4xl leading-none shadow-[var(--shadow-card)] backdrop-blur-sm dark:bg-card/10">
                  {entry.icon}
                </span>
              ) : (
                <div className="inline-flex size-12 items-center justify-center rounded-[var(--radius-lg)] border border-border/40 bg-card/70 shadow-[var(--shadow-card)] backdrop-blur-sm dark:bg-card/8">
        ...
        <div className="flex-1 overflow-y-auto">
          {visibleProps.length > 0 ? (
            <div className="px-4 py-4">
              ...
              {/* Property rows */}
              <div className="overflow-hidden rounded-[var(--radius-lg)] border border-border/50 bg-card shadow-[var(--shadow-card)] dark:bg-card/4">
                {visibleProps.map((prop, idx) => {
                  ...
                  return (
                    <div
                      key={prop.id}
                      className={[
                        "group/prop flex min-h-[36px] items-center transition-colors hover:bg-primary/[0.02]",
                    ...
                    >
                      ...
                      <div
                        className={[
                          "flex min-h-[36px] flex-1 cursor-pointer items-center px-3.5 py-2.5 text-sm transition-colors",
          ...
          <div className="px-4 pb-2">
            ...
            <div className="rounded-[var(--radius-lg)] border border-border/50 bg-card px-4 py-3 shadow-[var(--shadow-card)] dark:bg-card/4">
              <PageEditor
                pageId={entry.id}
                isLocked={false}
                isDeleted={false}
        ...
        {/* ── Footer CTA ──────────────────────────────────────────────────── */}
        <div className="shrink-0 bg-card/80 px-4 pb-5 pt-3 backdrop-blur-sm shadow-[0_-1px_0_rgba(0,0,0,0.06)] dark:bg-card/4">
          <Link
            href={`/app/${workspaceSlug}/${entry.shortId}`}
            className="group inline-flex items-center gap-2 rounded-[var(--radius-md)] bg-primary px-4 py-2 text-[13px] font-semibold text-white shadow-[var(--shadow-card)] transition-all hover:bg-[var(--primary-hover)] active:scale-[0.97]"
          >
            <FileText size={14} className="text-white" />
            Open full page
            <ArrowSquareOut
              size={13}
              className="shrink-0 text-white/70 transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5"
            />
          </Link>
        </div>
            ...
            >
              ...
              <div className="px-6 pb-8 pt-5">
                <div className="mb-5 flex items-start gap-4">
                  <div className="flex size-11 shrink-0 items-center justify-center rounded-[var(--radius-lg)] bg-red-100 dark:bg-red-950/50">
                    <Warning size={20} className="text-red-600 dark:text-red-400" weight="fill" />
                  </div>
                  <div>
                    <p className="text-base font-bold text-foreground">Delete this entry?</p>
                ...
                <div className="flex gap-3">
                  <button
                    ...
                    className="flex-1 rounded-[var(--radius-lg)] bg-red-600 py-3 text-sm font-bold text-white shadow-lg shadow-red-500/25 transition-all hover:bg-red-700 hover:shadow-[var(--shadow-raised)] disabled:opacity-60"
                  >
                    {deleting ? "Deleting…" : "Delete entry"}
                  </button>
                  <button
                    onClick={() => setConfirmDelete(false)}
                    className="flex-1 rounded-[var(--radius-lg)] border border-border bg-background py-3 text-sm font-semibold text-foreground transition-colors hover:bg-muted"
                  >
                    Keep it
                  </button>
                </div>
