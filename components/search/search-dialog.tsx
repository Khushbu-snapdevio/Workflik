  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        ...
        <svg className="size-3 opacity-60" viewBox="0 0 10 6" fill="currentColor">
      ...
      {open && (
        <div className="absolute left-0 top-full z-50 mt-1 min-w-[160px] rounded-xl border border-border bg-popover p-1 shadow-2xl">
          {options.map((opt) => (
            ...
            >
              {opt.value === value && (
                <svg className="size-3 text-primary" viewBox="0 0 12 12" fill="currentColor">
...
export function SearchDialog({ workspaceSlug, workspaceId, onClose }: SearchDialogProps) {
  ...
  return (
    <>
      {/* Backdrop */}
      <div
        className="fixed inset-0 z-[800] bg-black/40 backdrop-blur-[2px]"
        onClick={onClose}
      />
