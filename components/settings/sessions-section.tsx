  ...
  return (
    <div className="mx-auto max-w-[640px] px-8 py-10">
      ...
      <div className="mb-8 flex items-center gap-4">
        <div className="flex size-12 shrink-0 items-center justify-center rounded-[14px] bg-gradient-to-br from-[#059669] to-[#34d399] shadow-[0_4px_12px_rgba(5,150,105,0.35)]">
        ...
        <div>
          <h1 className="text-[22px] font-bold text-[#1c1917]">Security & sessions</h1>
          <p className="text-[13.5px] text-[#78716c]">Manage devices signed in to your account.</p>
        </div>
      </div>
      ...
      {current && (
        <div className="mb-7">
          <p className="mb-2 text-[10.5px] font-bold uppercase tracking-widest text-[#b3b0aa]">Current session</p>
          <div className="overflow-hidden rounded-[16px] border border-[#059669]/20 bg-gradient-to-br from-[#f0fdf4] to-white">
      ...
      {others.length > 0 && (
        <div className="mb-7">
          <div className="mb-2 flex items-center justify-between">
            <p className="text-[10.5px] font-bold uppercase tracking-widest text-[#b3b0aa]">
              Other sessions
              <span className="ml-2 rounded-full bg-[#f0f0ee] px-2 py-0.5 text-[10px] font-bold normal-case text-[#787774]">{others.length}</span>
            </p>
            <button type="button" onClick={revokeAll} disabled={revokingAll}
              className="flex items-center gap-1.5 text-[12.5px] font-semibold text-red-500 transition-colors hover:text-red-700 disabled:opacity-50">
          ...
          <div className="overflow-hidden rounded-[16px] border border-black/[0.07] bg-white">
            {others.map((s, i) => (
              <div key={s.id} className={i < others.length - 1 ? "border-b border-black/[0.05]" : ""}>
      ...
      {sessions.length === 0 && (
        <div className="flex flex-col items-center gap-3 py-16 text-center">
          <div className="flex size-14 items-center justify-center rounded-full bg-[#f0fdf4]">
            <svg viewBox="0 0 24 24" fill="none" stroke="#059669" strokeWidth="1.5" strokeLinecap="round" className="size-7">
              <path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/>
            </svg>
          </div>
          <p className="text-[14px] font-semibold text-[#37352f]">No active sessions</p>
          <p className="text-[13px] text-[#787774]">Sign in again to see your sessions here.</p>
        </div>
      )}

      {/* ── Security tip ── */}
      <div className="overflow-hidden rounded-[16px] border border-amber-200/60 bg-gradient-to-br from-amber-50/80 to-white">
        <div className="flex items-start gap-4 px-5 py-4">
          <div className="flex size-9 shrink-0 items-center justify-center rounded-[10px] bg-amber-100">
            <svg viewBox="0 0 16 16" fill="none" stroke="#b45309" strokeWidth="1.5" strokeLinecap="round" className="size-4">
              <circle cx="8" cy="8" r="6.5"/>
              <path d="M8 5v3.5"/><circle cx="8" cy="11" r=".5" fill="#b45309"/>
            </svg>
          </div>
          <div>
            <p className="text-[13px] font-semibold text-[#92400e]">Security tip</p>
            <p className="mt-0.5 text-[12.5px] text-[#78350f]">If you see an unfamiliar session, revoke it immediately and update your credentials.</p>
          </div>
        </div>
      </div>
    </div>
...
function SessionCard({ s, isCurrent, revoking, onRevoke }: {
  ...
  return (
    <div className="flex items-center gap-4 px-5 py-4">
      {/* Device icon */}
      <div className={`flex size-11 shrink-0 items-center justify-center rounded-[12px] ${isCurrent ? "bg-[#dcfce7]" : "bg-[#f5f4f2]"}`}>
        {mob ? (
          <svg viewBox="0 0 20 20" fill="none" stroke={isCurrent ? "#16a34a" : "#787774"} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="size-5">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex flex-wrap items-center gap-2">
          <p className="text-[14px] font-semibold text-[#1c1917]">{b}</p>
          {os && <span className="text-[13px] text-[#78716c]">on {os}</span>}
          {isCurrent && (
            <span className="rounded-full bg-green-100 px-2.5 py-0.5 text-[11px] font-bold text-green-700">● This device</span>
          )}
        </div>
        <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-0.5 text-[12px] text-[#78716c]">
          {s.ipAddress && (
      ...
      {!isCurrent && (
        <button type="button" onClick={onRevoke} disabled={revoking}
          className="shrink-0 rounded-[8px] border border-black/[0.1] bg-white px-3.5 py-1.5 text-[13px] font-medium text-[#37352f] transition-all hover:border-red-300 hover:bg-red-50 hover:text-red-600 disabled:opacity-50 active:scale-95">
          {revoking ? "…" : "Revoke"}
        </button>
      )}
    </div>
