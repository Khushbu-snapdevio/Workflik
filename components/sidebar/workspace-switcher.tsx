...
import { ChevronDown, Check, Plus, LogIn } from "lucide-react";
...
export function WorkspaceSwitcher({ currentSlug }: Props) {
 ...
 if (loading) {
  return (
   <div className="flex h-9 items-center gap-2 px-2">
    <span className="size-6 animate-pulse rounded-[var(--radius-xs)] bg-sidebar-accent" />
    <span className="h-3 w-24 animate-pulse rounded-[var(--radius-xs)] bg-sidebar-accent" />
   </div>
  );
 }
 ...
 return (
  <div className="relative">
   <button
    className="flex w-full items-center gap-2 rounded-[var(--radius-sm)] px-2 py-1.5 text-left transition-colors duration-150 hover:bg-sidebar-accent focus:outline-none"
   ...
   >
    ...
    <ChevronDown size={16} className={`shrink-0 text-sidebar-foreground/40 transition-transform duration-150 ${open ? "rotate-180" : ""}`} />
   </button>

   {open && (
    <>
     <div className="fixed inset-0 z-40" onClick={() => { setOpen(false); setShowJoin(false); }} />
     <div className="absolute left-0 top-full z-50 mt-1 w-72 overflow-hidden rounded-[var(--radius-sm)] border border-border bg-popover">
      ...
      <div className="p-1.5">
       ...
       {workspaces.map((ws) => {
        ...
        return (
         <button
          className={`flex w-full items-center gap-2.5 rounded-[var(--radius-sm)] px-2 py-2 text-left transition-colors duration-150 hover:bg-accent ${isActive ? "bg-muted" : ""}`}
          key={ws.id}
         ...
         >
          ...
          <span className="shrink-0 rounded-[var(--radius-xs)] bg-muted px-1.5 py-0.5 text-2xs font-semibold uppercase tracking-ui text-muted-foreground">
           {ws.role}
          </span>
      ...
      {showJoin && (
       <div className="border-t border-border p-3">
        ...
        <input
         ref={joinInputRef}
         className="mb-1.5 h-9 w-full rounded-[var(--radius-sm)] border border-border bg-background px-3 text-sm text-foreground placeholder:text-muted-foreground/50 focus:border-primary focus:outline-none"
        ...
        <div className="flex gap-2">
         ...
         <button
          className="flex h-8 items-center justify-center rounded-[var(--radius-sm)] border border-border px-3 text-xs text-muted-foreground hover:bg-accent"
      ...
      <div className={`border-t border-border p-1.5 ${showJoin ? "hidden" : ""}`}>
       <button
        className="flex w-full items-center gap-2 rounded-[var(--radius-sm)] px-2 py-2 text-sm font-medium text-muted-foreground transition-colors duration-150 hover:bg-accent hover:text-foreground"
...
function WorkspaceAvatar({
 ...
 return (
  <span className="grid size-6 shrink-0 place-items-center rounded-[var(--radius-sm)] bg-primary font-semibold text-primary-foreground text-xs">
