...
import {
  TableIcon, SquaresFourIcon, CalendarIcon, GridFourIcon,
  FunnelIcon, SortAscendingIcon, EyeIcon,
  PlusIcon, ImageIcon, SmileyStickerIcon,
  XIcon, TrashIcon, CheckIcon, HouseIcon,
  CaretRightIcon, ArrowSquareOutIcon,
  UserIcon, CalendarBlankIcon, TagIcon,
  LinkIcon, TextTIcon, NumberCircleOneIcon,
  CheckSquareIcon, ListBulletsIcon,
      ...
      <div className="grid max-h-[200px] grid-cols-10 gap-0.5 overflow-y-auto">
        {filtered.map((e) => (
          <button
            ...
            className="flex size-7 items-center justify-center rounded text-lg hover:bg-muted transition-colors"
          >
            {e}
          </button>
        ))}
...
function PanelPropValue({
  ...
  if (type === "checkbox") {
    ...
    return (
      ...
      >
        {checked && <CheckIcon size={10} weight="bold" className="text-primary-foreground" />}
      </button>
    );
  }
      ...
      {detailEntryId && (() => {
        ...
        return (
          <EntryDetailPanel
            ...
            onDelete={(id) => { deleteEntry(id); setDetailEntryId(null); }}
            onSaveTitle={savePanelTitle}
