...
The Pagevo editor is block-based — every piece of content is a discrete block that can be moved, transformed, nested, and styled independently. The editor is powered by **TipTap** (built on ProseMirror) and is the core interaction surface across all pages and database entries.
