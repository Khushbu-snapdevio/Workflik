...
Access control in Pagevo operates at two levels: **workspace roles** (what a member can do across the entire workspace) and **page-level permissions** (what they can do on a specific page). These two layers combine to give precise control without complexity.
...
- The Pagevo platform team (Platform Admins) can view a list of private page **titles only** (not content) via **Orbit Admin** for compliance purposes — this is **not** available to workspace Admins
...
Share a page with anyone on the internet — no Pagevo account required.
...
- **Non-public subpage links:** If a visitor on a public page clicks a link to a subpage that has no public link enabled, they are shown a `"This page is not publicly available"` screen with a `"Sign in to Pagevo"` button. It is never a 404 — the page exists, it is simply not shared publicly.
...
3. If the guest has no Pagevo account, they complete a magic-link sign-in with their invited email (auto-creates the account).
...
Beyond workspace-level permissions, the Pagevo platform team uses **Orbit Admin** for cross-workspace operations. These capabilities are not available to any end user, including workspace Admins.
