...
Local dev startup that day hit a Postgres port collision: the embedded Postgres instance for a *different* local project (Kanbanica) was already bound to port 5432, the same port Pagevo's `.env` `DATABASE_URL` uses. `pnpm db:local` failed twice before the port freed up and Pagevo's own embedded Postgres finally started.
