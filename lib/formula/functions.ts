...
export const FORMULA_FUNCTIONS: Record<string, (args: FormulaValue[]) => FormulaValue> = {
  // ── Math ──────────────────────────────────────────────────────────────────
  ...
  slice:   (a) => toStr(a[0]).slice(toNumber(a[1], "slice"), a[2] != null ? toNumber(a[2], "slice") : undefined),
  replace: (a) => toStr(a[0]).split(toStr(a[1])).join(toStr(a[2])),

  // ── Date ──────────────────────────────────────────────────────────────────
  now:        () => new Date(),
  dateAdd:      (a) => new Date(toDate(a[0], "dateAdd").getTime() + toNumber(a[1], "dateAdd") * (MS_PER_UNIT[toStr(a[2])] ?? MS_PER_UNIT.days)),
  dateSubtract:   (a) => new Date(toDate(a[0], "dateSubtract").getTime() - toNumber(a[1], "dateSubtract") * (MS_PER_UNIT[toStr(a[2])] ?? MS_PER_UNIT.days)),
  formatDate:    (a) => toDate(a[0], "formatDate").toLocaleDateString(undefined, { year: "numeric", month: "short", day: "numeric" }),
  dateBetween:   (a) => {
    const ms = toDate(a[0], "dateBetween").getTime() - toDate(a[1], "dateBetween").getTime();
  ...
  not: (a) => !isTruthy(a[0]),
  empty: (a) => a[0] == null || a[0] === "" || (typeof a[0] === "number" && Number.isNaN(a[0])),
};
