...
export async function handleCleanupStaleUploads(_jobs: Job<Record<string, never>>[]) {
  ...
  const stale = await db
    ...
    .from(fileUploads)
    .where(and(isNull(fileUploads.confirmedAt), lt(fileUploads.createdAt, cutoff)));
  ...
  await db
    ...
    .where(
      and(
        ...
        inArray(fileUploads.id, stale.map((r) => r.id)),
      ),
    );
