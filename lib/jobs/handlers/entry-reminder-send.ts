...
export async function handleEntryReminderSend(
  ...
  const due = await db
    ...
    .innerJoin(databaseProperties, eq(databaseProperties.id, entryReminders.propertyId))
    .where(and(eq(entryReminders.notified, false), lte(entryReminders.remindAt, new Date())))
    .limit(200);
  ...
  for (const reminder of due) {
    await db.transaction(async (tx) => {
      ...
      const [updated] = await tx
        ...
        .set({ notified: true })
        .where(and(eq(entryReminders.id, reminder.id), eq(entryReminders.notified, false)))
        .returning({ id: entryReminders.id });

      if (!updated) return; // Another concurrent run already handled this reminder
