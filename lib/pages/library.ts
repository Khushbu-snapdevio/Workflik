...
import { pageClosure, pages, userFavorites, userRecentlyVisited, users } from "@/lib/db/schema";
import { DEFAULT_PAGE_SIZE, MAX_PAGE_SIZE, MIN_PAGE_SIZE } from "@/lib/ui/pagination";
...
export async function getLibraryPage(
  workspaceId: string,
  userId: string,
  opts: { tab?: string | null; search?: string; page?: number; pageSize?: number },
): Promise<LibraryPageResult> {
  const tab: LibraryTab =
    opts.tab === "recents" || opts.tab === "favorites" || opts.tab === "private" ? opts.tab : "all";
  const search = (opts.search ?? "").trim();
  const pageNum = opts.page && Number.isFinite(opts.page) && opts.page > 0 ? opts.page : 1;
  const pageSize = Math.min(MAX_PAGE_SIZE, Math.max(MIN_PAGE_SIZE, opts.pageSize && Number.isFinite(opts.pageSize) ? opts.pageSize : DEFAULT_PAGE_SIZE));
  ...
  if (nestingActive) {
    ...
    const descendantRows = rootIds.length > 0
      ? await db
          .select(selectCols)
          .from(pageClosure)
  ...
  const [{ value: allCount }] = await db.select({ value: count(pages.id) }).from(pages)
    .where(and(eq(pages.workspaceId, workspaceId), eq(pages.isDeleted, false)));
  const [{ value: recentsCount }] = recentPageIds.length > 0
    ? await db.select({ value: count(pages.id) }).from(pages)
        .where(and(eq(pages.workspaceId, workspaceId), eq(pages.isDeleted, false), inArray(pages.id, recentPageIds)))
    : [{ value: 0 }];
  const [{ value: favoritesCount }] = favPageIds.length > 0
    ? await db.select({ value: count(pages.id) }).from(pages)
        .where(and(eq(pages.workspaceId, workspaceId), eq(pages.isDeleted, false), inArray(pages.id, favPageIds)))
    : [{ value: 0 }];
  const [{ value: privateCount }] = await db.select({ value: count(pages.id) }).from(pages)
    .where(and(eq(pages.workspaceId, workspaceId), eq(pages.isDeleted, false), eq(pages.isPrivate, true)));
