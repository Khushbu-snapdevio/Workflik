...
export function useAnchorPosition({
  ...
  useLayoutEffect(() => {
    ...
    // `refs.setReference` is stable (Floating UI memoizes it), so listing it is
    // inert — the loop described above came from `anchorRect`, which is why only
    // its decomposed numeric fields are depended on here.
  }, [element, top, left, right, bottom, refs.setReference]);
